#!/usr/bin/env bash
# Push fft-2048 (top: fftmain_wrapper) through circt-verilog and arcilator.
set -euo pipefail
...
HW_MLIR="${BUILD_DIR}/fftmain_wrapper_hw.mlir"
STATE_JSON="${BUILD_DIR}/fftmain_wrapper_state.json"
LLVM_IR="${BUILD_DIR}/fftmain_wrapper.ll"
OBJECT_FILE="${BUILD_DIR}/fftmain_wrapper.o"
...
echo "==> Importing fft-2048 design to HW MLIR (top=${TOP_MODULE})"
"${CIRCT_VERILOG}" --ir-hw --top="${TOP_MODULE}" --sroa --detect-memories=0 \
  -o "${HW_MLIR}" "${DESIGN_SOURCES[@]}"
